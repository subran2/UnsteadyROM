...
using WaterLily, StaticArrays, PlutoUI, Interpolations, Plots, Images,LsqFit, DelimitedFiles, FileIO

# ╔═╡ b0c8df66-1e63-456e-be32-d469e6972e00
md"""
## Simulation of unsteady pitching airfoil NACA0012
...
# ╔═╡ d7eff2bd-9e4a-4433-9695-c0c93f873257
begin
...
begin
	 ...
	 Maᵣₑₐₗ = 0.0162 # Ma number of airflow
	 Uᵣₑₐₗ= Maᵣₑₐₗ*cᵣₑₐₗ # free stream velocity of air [m/s]
...
# ╔═╡ c314b127-1be4-4887-bc43-a3495be1a198

	md"""
	`Re`,`a`,`f`,`αₘₑₐₙ`,`αₐₘₚ` are specified. Additonal parameters such as `runtime` and `time step` can be changed.  `F_thrust` and `F_side` are scaled forces which are assumed to dimensionless parameters. The scaled forces are thus be decomposed into `lift coefficient Cl` and `drag coefficient Cd` respestively. `Cl = - F_thrust sin α  + F_side cos α` and  `Cd = F_thrust cos α  + F_side sin α`
	
	"""

# ╔═╡ 56559e3e-51a8-47d7-815e-2af8c4c6c40f
begin
	...
	runtime = 30*period
	timesteps = 300
	time_vec = collect(range(0,runtime,timesteps))
	forces = []
	for j in time_vec
			push!(forces, get_force(airfoil_sim, j))
	end 
	...
	α = αₘₑₐₙ .+ αₐₘₚ.*cos.((ωᵩ.*time_vec) .- 0.5π)
	F_lift = -(F_thrust.*sin.(α))+(F_side.*cos.(α))
	F_drag = (F_thrust.*cos.(α))+(F_side.*sin.(α))
...
# ╔═╡ 75b782ed-e84e-4b1c-a87a-013712f65b90

md"""
##### Plotting the simulation

The `lift coefficient Cl` and `drag coefficient Cd` plotted against time. Replace  `*` with folder designation for saving the datafile and plots
 """

# ╔═╡ eafe8b07-fa6c-4756-97f8-0c06a1b53ad6
begin

αₐₘₚ_deg = rad2deg(αₐₘₚ)
αₘₑₐₙ_deg = rad2deg(αₘₑₐₙ)
...
md"""
A gif of the vorticity `ω=curl(u)` to visualize the vortices in the wake of the airfoil is created. `Re = 1e5,  a=0.25 (COR is 25% from LE),  f = 1,  αₘₑₐₙ = 4°,  αₐₘₚ = 4°` are used for the visualization. Uncomment code and replace `*` with folder designation for saving the gif
"""
...
# 	airfoil_sim_gif = airfoil(1e5,0.25,1,deg2rad(4),deg2rad(4))


# 	anim =	@animate for t ∈ collect(range(0, 5, step=0.2))
# 		sim_step!(airfoil_sim_gif,t,remeasure=true)
# 		plot_vorticity(airfoil_sim_gif)
# 		end
#    gif(anim,"*/sample unsteady pitching airfoil.gif",fps=10)
